...
def train(config):
    ...
    print(f"Starting TQC training for {num_seeds} seeds...")

    for idx, seed in enumerate(config['seeds']):
        env = make_env(config['env'], seed)
        model = TQC('MlpPolicy', env, seed=seed, verbose=0)

        eval_callback = EvalCallback(
            env,
            best_model_save_path=f"evaluation_study/logs/{config['env']}_TQC_{seed}/",
            log_path=f"evaluation_study/logs/{config['env']}_TQC_{seed}/",
            eval_freq=100,
            deterministic=True,
            render=False
        )
        ...
        eval_npz_path = os.path.join(f"evaluation_study/logs/{config['env']}_TQC_{seed}", "evaluations.npz")
        if os.path.exists(eval_npz_path):
            ...
            save_reward_curve_as_csv(config['env'], "tqc", seed, timesteps, rewards)
        else:
    ...
    save_results(config['env'], 'tqc', all_returns)
