...
def train(config):
    ...
    print(f"Starting DDPG training for {num_seeds} seeds...")
    ...
    for idx, seed in enumerate(config['seeds']):
        ...
        eval_callback = EvalCallback(
            env,
            best_model_save_path=f"evaluation_study/logs/{config['env']}_DDPG_{seed}/",
            log_path=f"evaluation_study/logs/{config['env']}_DDPG_{seed}/",
            eval_freq=100,
            deterministic=True,
            render=False
        )
        ...
        eval_npz_path = os.path.join(f"evaluation_study/logs/{config['env']}_DDPG_{seed}", "evaluations.npz")
        if os.path.exists(eval_npz_path):
            ...
            save_reward_curve_as_csv(config['env'], "ddpg", seed, timesteps, rewards)
        else:
    ...
    save_results(config['env'], 'ddpg', all_returns)
