...
def train(config):
    ...
    print(f"Starting A2C training for {num_seeds} seeds...")
    ...
    for idx, seed in enumerate(config['seeds']):
        ...
        eval_callback = EvalCallback(
            env,
            eval_freq=100,
            best_model_save_path=f"evaluation_study//logs/{config['env']}_A2C_{seed}/",
            log_path=f"evaluation_study/logs/{config['env']}_A2C_{seed}/",
            deterministic=True,
            render=False
        )
        ...
        eval_npz_path = os.path.join(f"evaluation_study/logs/{config['env']}_A2C_{seed}", "evaluations.npz")
        if os.path.exists(eval_npz_path):
            ...
            save_reward_curve_as_csv(config['env'], "a2c", seed, timesteps, rewards)
        else:
    ...
    save_results(config['env'], 'a2c', all_returns)
